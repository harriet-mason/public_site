...
???
- Hi everyone, I'm Harriet Mason, a PhD student at Monash University, it is 4am here, I decided to stay up late instead of wake up early and I now think that was a terrible decision, so we will see how this goes. 
- Today I'm going to be talking about scagnostics
- What are scagnostics you may be thinking,
- They are a group of measures that evaluate the visual features of a scatter plot.
- Your next question might be, well why do you care about them?
- Well, scatterplots are particularly useful for examining all kinds of association between variables, 
- but unfortunately big data has too many variables to plot them all.
- So, what if instead of looking at every pairwise plot, we instead picked out an interesting subst and only looked at those? That is the main idea behind scagnostics
...
- To understand some of these scagnostics it helps to see some examples, so lets go through three of them
...
- The name is also an acronym and it stands for CAlculate Scagnostics on Scatterplots Over Wads of Associated Real numberYs in R
- I know what you are thinking, and no, we definiately didnt come up with the acronym after the name
...
<iframe src="aflwscag.html" width="800" height="500" scrolling="yes" seamless="seamless" frameBorder="0"> </iframe>
...
- the top_scags() function gives the top scagnostic for each scatter plot,
...
```{r, echo=TRUE}
#scags with short named scatter plots so they fit
top_scags(AFLW_scags)[-c(3,4),] 
```

???
- top_scags() gives you the scatter plot that had the highest value on that scatter plot for each scagnostic
...
- and a couple weeks ago they released interp wwhich is an open source version of tripack, and a few days ago updated alphahull to rely on interp
- So... now I have to swap all the code over to using interp instead of tripack. 
- Fun week
---
...
- After that I'll re-enter the hellish landscape that is trying to get a package on CRAN
- A version two of the package, will probably have faster calculated, especially for the clumpy scagnostic, ill also try to introduce hexagonal binning so the original scagnostics aren't essentially useless.
